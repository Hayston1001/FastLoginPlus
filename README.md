...
The panel requires Java 17+. On Java 8, the Bukkit plugin still loads but skips
the panel. Folia does not provide the panel.
...
- The token **is** the full admin access to the panel. Protected `/api/*` endpoints
...
- Bind `host` to `127.0.0.1` (or a LAN interface) when the panel does not
  need internet access. A blank host also falls back to `127.0.0.1`.
...
`Authorization: Bearer <token>` header. The language endpoints
(`/api/lang` and `/api/lang/{code}`) are public exceptions.
