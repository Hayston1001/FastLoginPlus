...
// ── State ──────────────────────────────────────────
// The real login token persists in localStorage; the demo token is
...
// Adaptive status polling (backoff after 429/5xx)
...
async function api(endpoint, options = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            // Judge the logout by the HTTP status code, not by a body.error
            // string - the endpoint JSON may overwrite the body and the error
...
(async () => {
    ...
    // Adaptive polling - a fixed 30s interval kept hammering a
...
function logout() {
    ...
    // Demo mode lives in sessionStorage (per browser session) — clear both the
    // demo flag and the session-scoped demo token
...
function getOnlineActions(player) {
    switch (player.type) {
        ...
        case 'cracked':
            return `<span class="btn-group"><button class="btn-action btn-danger-action" data-action="delete" data-name="${escapeHtml(player.name)}">${I18n.t('online.action.delete')}</button><button class="btn-action btn-success" data-action="premium" data-name="${escapeHtml(player.name)}">${I18n.t('online.action.switchPremium')}</button></span>`;
        default:
            return '—';
    }
}
...
// Stale-response guard - concurrent player searches previously raced and the
// slowest (not the newest) response won
...
function escapeHtml(text) {
    if (!text) return '';
    // Escape &, <, > and both quote styles: values are also interpolated into
    // HTML attributes, and the innerHTML trick does not escape quotes.
    return String(text)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
function getFloodgateBadge(state) {
    switch (state) {
        case 'FALSE':  return `<span class="badge badge-java">${I18n.t('floodgate.java')}</span>`;
...
function formatDuration(ms) {
    // Missing/NaN inputs would render "NaNd 5h" style garbage
