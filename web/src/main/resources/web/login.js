...
(async () => {
    ...
    // Already have a token → go to dashboard (a demo session counts)
...
function handleLogin() {
    ...
    // Demo mode (session-scoped - no cross-session residue)
    if (input === DEMO_TOKEN) {
    ...
    fetch('/api/status', {
        ...
        if (resp.ok) {
            ...
            // Distinguish failure classes instead of a blanket 'Invalid token'
